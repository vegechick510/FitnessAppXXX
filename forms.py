...
class RegistrationForm(FlaskForm):
    ...
    height = FloatField(
        "Height", validators=[Optional(), NumberRange(min=100, max=250)]
    )
    goal = StringField("Goal", validators=[Optional(), Length(min=2, max=50)])
    target_weight = FloatField(
        "Target Weight", validators=[Optional(), NumberRange(min=20, max=200)]
    )
    coach = SelectField(
        "Coach", choices=[], validators=[Optional()]
    )  # Dropdown for student to select a coach
    ...
    experience = FloatField(
        "Experience (years)", validators=[Optional(), NumberRange(min=0)]
    )
    ...
    def validate(self):
        ...
        if self.user_type.data == "student":
            ...
            missing_fields = [
                field.label.text for field in required_fields if not field.data
            ]
            ...
            if missing_fields:
                for field in required_fields:
                    if not field.data:
                        field.errors.append(
                            f"{field.label.text} is required for students."
                        )
            ...
            if not self.specialization.data:
                self.specialization.errors.append(
                    "Specialization is required for coaches."
                )
            if self.experience.data is None or self.experience.data < 0:
                self.experience.errors.append(
                    "Experience is required for coaches and must be a non-negative number."
                )

            if self.specialization.errors or self.experience.errors:
                print(
                    "Coach field errors:",
                    self.specialization.errors,
                    self.experience.errors,
                )
                return False
...
class ProgressForm(FlaskForm):
    ...
    current_weight = DecimalField(
        ...
        validators=[
            Optional(),
            NumberRange(
                min=0, max=500, message="Current Weight must be between 0 and 500 kg"
            ),
        ],
    )
    ...
    goal_weight = DecimalField(
        ...
        validators=[
            Optional(),
            NumberRange(
                min=0, max=500, message="Goal Weight must be between 0 and 500 kg"
            ),
        ],
    )
    ...
    waist = DecimalField(
        ...
        validators=[
            Optional(),
            NumberRange(min=50, max=150, message="Waist must be between 50 and 150 cm"),
        ],
    )
    hips = DecimalField(
        "Hips (cm)",
        validators=[
            Optional(),
            NumberRange(min=70, max=160, message="Hips must be between 70 and 160 cm"),
        ],
    )
    chest = DecimalField(
        "Chest (cm)",
        validators=[
            Optional(),
            NumberRange(min=70, max=500, message="Chest must be between 70 and 150 cm"),
        ],
    )
...
class ReminderForm(FlaskForm):
    ...
    reminder_type = SelectField(
        "Reminder Type",
        choices=[("goal", "Goal"), ("workout", "Workout")],
        validators=[DataRequired()],
    )
...
class StreakForm(FlaskForm):
    ...
    current_streak = IntegerField(
        "Current Streak (days)", validators=[Optional()], render_kw={"readonly": True}
    )
...
class MoodForm(FlaskForm):
    ...
    mood = TextAreaField(
        ...
        validators=[
            DataRequired(),
            Length(max=500, message="Mood must be less than 500 characters."),
        ],
    )
    submit = SubmitField("Submit")
